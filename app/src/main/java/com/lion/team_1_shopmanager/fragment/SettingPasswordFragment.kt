...
import com.lion.team_1_shopmanager.MainActivity.FragmentName
import com.lion.team_1_shopmanager.databinding.FragmentSettingPasswordBinding
import com.lion.team_1_shopmanager.util.Value.hideSoftInput
import com.lion.team_1_shopmanager.util.Value.setLastComplete
...
class SettingPasswordFragment : Fragment() {
    ...
    fun settingInitView(){
        fragmentSettingPasswordBinding.apply {
            ...
            mainActivity.showSoftInput(textFieldSettingPassword.editText!!)
        }
    }

    fun setEditAction(){
        fragmentSettingPasswordBinding.apply {
            textFieldSettingPassword.editText?.setOnEditorActionListener { v, actionId, event ->
                textFieldSettingPasswordCheck.requestFocus()
                false
            }
            setLastComplete(textFieldSettingPasswordCheck.editText!!,buttonSettingPasswordSubmit)
        }
...
//                builder1.show()
    ...
    fun processingInputManagerPassword(){
        fragmentSettingPasswordBinding.apply {
            // 입력한 비밀번호를 가져온다.
            val pw1 = textFieldSettingPassword.editText?.text!!.toString()
            val pw2 = textFieldSettingPassword.editText?.text!!.toString()
            ...
            if(pw1.isEmpty()){
                textFieldSettingPassword.error = "비멀번호를 입력해주세요"
                inputFlag = false
            }

            // 두 번째 비밀번호에 입력한 것이 없다면
            if(pw2.isEmpty()){
                textFieldSettingPassword.error = "비밀번호를 입력해주세요"
                inputFlag = false
            }

            // 두 비밀번호가 서로 다르다면
            if(pw1.isNotEmpty() && pw2.isNotEmpty() && pw1 != pw2){
                textFieldSettingPassword.error = "입력하신 비빌번호가 서로 다릅니다"
                textFieldSettingPassword.error = "입력하신 비빌번호가 서로 다릅니다"

                textFieldSettingPassword.editText?.setText("")
                textFieldSettingPassword.editText?.setText("")

                textFieldSettingPassword.editText?.requestFocus()
                inputFlag = false
            }
